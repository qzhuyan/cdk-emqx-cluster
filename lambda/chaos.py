...
def prom_query(url:str, query:str, time: time.time):
    ...
    if resp.status_code==200 and resp.json()['status'] == 'success':
        ...
        Fail("prom_query failed with status: {resp.status_code}")
...
def check_traffic(prom_host:str, time_at:time.time, period:str='5m'):
    ...
    for metric_name in ['emqx_client_subscribe',
        ...
        if float(res[0]['value'][1]) == 0:
            Fail("Error: traffic error: {{metric_name}} is 0")
        ...
        for r in res:
            ...
            v=r['value'][1]
            if float(v) == 0:
                Fail(f"Error: {ins}'s {metric_name} is 0")
...
def inject_fault(cluster_name:str, fault_name:str):
    ...
    if not fid:
        Fail(f"Error: inject_fault, fault id not found for {fault_name}")
